...
import ITeamDrawUseCase from "./ITeamDrawUseCase";

export default class TeamDrawUseCase implements ITeamDrawUseCase {
  ...
  async execute(players: Player[], maximumTeamPlayers: number): Promise<Team[]> {
    const positions = await this.positionRepository.get();
